...
await wait(100)
const effectDetail = await evaluate(`document.querySelector('.raw-effect-list')?.textContent.includes('爆擊率') && !document.querySelector('.detail-grid')?.textContent.includes('孔數') && !document.querySelector('.detail-gamble-link')`)
if (!effectDetail) throw new Error('Localized raw equipment effects were not shown, sockets remain in the detail drawer, or a socketable exposed gambling navigation')
