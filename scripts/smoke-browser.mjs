...
await wait(100)
const effectDetail = await evaluate(`(() => { const drawer=document.querySelector('.detail-drawer'); return Boolean(document.querySelector('.raw-effect-list')?.textContent.includes('爆擊率') && drawer?.textContent.includes('SOCKETS: 0') && drawer?.textContent.includes('RARITY: 0') && !drawer?.textContent.includes('孔數: 0') && !drawer?.textContent.includes('稀有度: 獨特') && !document.querySelector('.detail-gamble-link')); })()`)
if (!effectDetail) throw new Error('Localized effects, source socket/RARITY metadata, or socketable gambling navigation is incorrect')
...
await wait(160)
const equipmentGamblePreset = await evaluate(`(() => { const inputs=document.querySelectorAll('.gamble-controls input'); return location.hash==='#/gambling/chest/105/2' && document.querySelector('.gamble-controls .select-trigger')?.textContent.includes('胸甲') && inputs[0]?.value==='105' && inputs[0]?.max==='105' && inputs[1]?.value==='2'; })()`)
if (!equipmentGamblePreset) throw new Error('Equipment details did not navigate to gambling with its mapped type, level and original socket count')
await evaluate(`location.hash='#/items'`)
await wait(180)
