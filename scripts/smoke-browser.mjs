...
await evaluate(`document.querySelector('.data-table tbody tr')?.click()`)
await wait(100)
const weaponSpeed = await evaluate(`document.querySelector('.detail-grid')?.textContent.includes('攻擊速度')`)
if (!weaponSpeed) throw new Error('Weapon attack speed was not rendered')
await evaluate(`document.querySelector('.drawer-close')?.click()`)
...
await evaluate(`document.querySelectorAll('.gear-slot')[5]?.querySelector('.slot-empty')?.click()`)
await wait(100)
