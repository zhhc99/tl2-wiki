...
assert(equipment.every(row=>row.rarityValue===null||Number.isInteger(row.rarityValue)),'Equipment contains an invalid source RARITY value')
assert(equipment.every(row=>Number.isInteger(row.value)&&row.value>0),'Equipment contains an invalid VALUE')
...
assert(equipment.find(row=>row.name.en==="Arcgap's Vice")?.rarityValue===35,'Arcgap\'s Vice source RARITY value is missing')
assert(equipment.find(row=>row.name.en==='Heartsalve')?.value===80,'Heartsalve inherited VALUE override is missing')
