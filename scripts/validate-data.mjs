...
assert(equipment.length===3840&&equipment.length===meta.counts.equipment,'Expected 3,840 non-normal player-facing equipment records')
...
assert(equipment.reduce((sum,row)=>sum+(row.rawEffects.length||row.effects.length),0)===meta.counts.itemEffects,'Displayed item effect count differs from meta.json')
...
assert(equipment.every(row=>row.rawEffects.every(effect=>effect.text==null||localeComplete(effect.text))),'Equipment contains an incomplete localized raw effect')
