...
const gamblingPreset = (items: EquipmentIndexEntry[], search: string) => {
  ...
  return {
    ...
    sockets: item
      ? boundedInteger(String(item.sockets), 0, 0, 5)
      : boundedInteger(requestedSockets, 0, 0, 5),
    itemId: item?.id ?? null,
  }
}
...
export function GamblingPage({
  ...
  const chooseItem = (item: EquipmentIndexEntry) => {
    ...
    setType(itemType)
    setLevel(item.level)
    setSockets(boundedInteger(String(item.sockets), 0, 0, 5))
    setItemId(item.id)
    setPickerOpen(false)
    setQuery('')
  }
  ...
  return (
    <>
      ...
      <div className="content page-body gamble-page">
        <div className="gamble-layout">
          <section className="gamble-controls">
            <div className="gamble-mark">
              <CircleDollarSign />
              <span>{copy(lang, '价格计算器', 'Price calculator', '價格試算')}</span>
            </div>
            <label>
              <span>{copy(lang, '物品类型', 'Item type', '物品類型')}</span>
              <SelectControl
                className="planner-select"
                label={copy(lang, '物品类型', 'Item type', '物品類型')}
                value={type}
                onChange={changeType}
                options={gambleTypes.map((row) => ({
                  value: row.id,
                  label: copy(lang, row.zhCN, row.en, row.zhTW),
                }))}
              />
            </label>
            <div className="gamble-item-field">
              <span>{copy(lang, '具体物品', 'Specific item', '指定物品')}</span>
              <div className={`gamble-item-selection${selectedItem ? ' selected' : ''}`}>
                <button
                  onClick={() => {
                    setQuery('')
                    onLoadItems()
                ...
                >
                  {selectedItem ? (
                    <>
                      <img
                        className={`rarity-border ${selectedItem.rarity}`}
                        src={asset(selectedItem.iconPath)}
                        alt=""
                      />
                      <span>
                        <b>
                          {pick(selectedItem.name, lang)} <NgBadge tier={selectedItem.ngTier} />
                        </b>
                        <small>
                          Lv {selectedItem.level} · VALUE {selectedItem.value}
                        </small>
                      </span>
                    </>
                  ) : (
                    <span>
                      <b>{copy(lang, '选择具体物品', 'Choose a specific item', '選擇指定物品')}</b>
                      <small>
                        {copy(
                          lang,
                          '不选择时使用物品类型的基础值',
                ...
                {selectedItem && (
                  <button
                    className="clear-gamble-item"
                    onClick={() => setItemId(null)}
                    aria-label={copy(lang, '清除具体物品', 'Clear specific item', '清除指定物品')}
                  >
                    <X size={16} />
                  </button>
                )}
            ...
            <label>
              <span>{copy(lang, '物品等级', 'Item level', '物品等級')}</span>
              <NumberInput min={1} max={105} value={level} onChange={setLevel} />
            </label>
            <label>
              <span>{copy(lang, '孔数', 'Sockets', '孔數')}</span>
              <NumberInput min={0} max={5} value={sockets} onChange={setSockets} />
            </label>
          </section>
          <section className="price-results">
            <header>
              <div>
                <span>{copy(lang, '可能价格', 'Possible prices', '可能價格')}</span>
                <h2>
                  {selectedItem
                    ? pick(selectedItem.name, lang)
                    : copy(lang, selectedType.zhCN, selectedType.en, selectedType.zhTW)}{' '}
                  · Lv {level}
                </h2>
              </div>
            ...
            <div className="price-table">
              ...
              {rows.map((row) => (
                <div className="price-row" key={row.offset}>
                  <span>Lv + {row.offset}</span>
        ...
        <section className="gamble-notes">
          <Swords />
          <div>
            <h2>{copy(lang, '价格计算方式', 'Price calculation', '價格計算方式')}</h2>
            <p>
              <span>
                {copy(
                  lang,
              ...
              {copy(
                lang,
                '；每个孔会使价格提高 10%。再将随机值计入物品等级，',
                '; every socket increases the price by 10%. The random roll is then applied to the item level to ',
                '；每個孔會讓價格提高 10%。再將隨機值計入物品等級，',
              )}
              <span>{copy(lang, '得到四档价格', 'produce four prices', '得到四種價格')}</span>
      ...
      {pickerOpen && (
        <div
          className="picker-backdrop gamble-picker-backdrop"
          onMouseDown={(event) => {
        ...
        >
          <section
            ...
            role="dialog"
            aria-modal="true"
            aria-label={copy(lang, '选择具体物品', 'Choose a specific item', '選擇指定物品')}
          >
            <header>
              <div>
                <span>{copy(lang, selectedType.zhCN, selectedType.en, selectedType.zhTW)}</span>
                <h2>{copy(lang, '选择具体物品', 'Choose a specific item', '選擇指定物品')}</h2>
              </div>
              <button
                onClick={() => setPickerOpen(false)}
                aria-label={copy(lang, '关闭', 'Close', '關閉')}
              >
                <X />
              </button>
            </header>
            ...
            <div className="picker-list">
              ...
              >
                {copy(lang, '不选择具体物品', 'No specific item', '不選擇指定物品')}
              </button>
              {pickerItems.map((item) => (
                <button key={item.id} onClick={() => chooseItem(item)}>
                  <img
                  ...
                  <span>
                    ...
                    <small>
                      Lv {item.level} · VALUE {item.value}
