...
export function SelectControl({
  ...
  const onKeyDown = (event: KeyboardEvent<HTMLButtonElement>) => {
    ...
    if (event.key.length === 1 && !event.ctrlKey && !event.metaKey && !event.altKey) {
      ...
      const match = options.findIndex(
        (option) =>
          !option.disabled && option.label.toLocaleLowerCase().startsWith(typeahead.current.value),
      )
      if (match >= 0) {
  ...
  return (
    <div
      ref={rootRef}
      className={`select-control${open ? ' is-open' : ''}${icon ? ' has-icon' : ''}${className ? ` ${className}` : ''}`}
    >
      {icon && (
      ...
      <button
        ...
        onKeyDown={onKeyDown}
      >
        <span className="select-value">{selected?.label || String(value)}</span>
        <ChevronDown className="select-caret" size={14} aria-hidden="true" />
      </button>
      {open && (
        <div id={listboxId} className="select-menu" role="listbox" aria-label={label}>
          {options.map((option, index) => (
            <button
              ...
              className={`select-option${index === selectedIndex ? ' is-selected' : ''}`}
              key={option.value}
