...
function RawEffectLine({effect,lang,pieces}:{effect:RawEffect;lang:Lang;pieces?:number}){
  const label=effectNames[effect.type]?pick(effectNames[effect.type],lang):titleCase(effect.type)
...
function ItemsPage({lang,items}:{lang:Lang;items:DbEquipment[]}){
  ...
  const filtered=useMemo(()=>items.filter(item=>(category==='all'||item.category===category)&&(rarity==='all'||item.rarity===rarity)&&(level==='all'||(level==='100'?item.level>=100:item.level>=Number(level)&&item.level<Number(level)+20))&&(`${allText(item.name)} ${item.subtype} ${item.set?allText(item.set):''} ${item.effects.map(effect=>allText(effect.text)).join(' ')}`).toLowerCase().includes(query.toLowerCase())),[items,category,rarity,level,query])
  ...
  return <><PageHeader section={tr(lang,'navItems')} title={tr(lang,'itemsTitle')}>{copy(lang,'按名称、类型、稀有度、等级或物品效果查找装备。','Find equipment by name, type, rarity, level or item effect.','按名稱、類型、稀有度、等級或物品效果查找裝備。')}</PageHeader><div className="content page-body"><div className="data-toolbar"><label className="data-search"><Search size={16}/><input value={query} onChange={event=>setQuery(event.target.value)} placeholder={copy(lang,'搜索名称、类型、套装或效果…','Search name, type, set or effect…','搜尋名稱、類型、套裝或效果…')}/></label><SelectControl className="filter-select" label={copy(lang,'装备类型','Equipment type','裝備類型')} value={category} onChange={event=>setCategory(event.target.value as typeof category)}>{['all','weapon','armor','trinket','pet','socketable'].map(value=><option value={value} key={value}>{tr(lang,categoryKey(value))}</option>)}</SelectControl><SelectControl className="filter-select" label={copy(lang,'稀有度','Rarity','稀有度')} value={rarity} onChange={event=>setRarity(event.target.value as typeof rarity)}><option value="all">{tr(lang,'allRarity')}</option>{(['normal','rare','unique','legendary'] as Rarity[]).map(value=><option value={value} key={value}>{rarityName(value,lang)}</option>)}</SelectControl><SelectControl className="filter-select" label={copy(lang,'装备等级','Item level','裝備等級')} value={level} onChange={event=>setLevel(event.target.value)}><option value="all">{copy(lang,'全部等级','All levels','全部等級')}</option>{[0,20,40,60,80,100].map(value=><option value={value} key={value}>Lv {value}{value<100?`–${value+19}`:'+'}</option>)}</SelectControl></div>
    <div className="result-meta"><span>{filtered.length.toLocaleString()} {tr(lang,'itemsFound')}</span><span>{copy(lang,`第 ${currentPage} / ${pages} 页`,`Page ${currentPage} of ${pages}`,`第 ${currentPage} / ${pages} 頁`)}</span></div>{!items.length?<Loading lang={lang}/>:<div className="table-scroll"><table className="data-table"><thead><tr><th>{copy(lang,'名称','Name','名稱')}</th><th>{copy(lang,'类型','Type','類型')}</th><th>{copy(lang,'稀有度','Rarity','稀有度')}</th><th>{tr(lang,'level')}</th><th>{tr(lang,'required')}</th><th>{tr(lang,'sockets')}</th><th/></tr></thead><tbody>{rows.map(item=><tr key={item.id} onClick={()=>setSelected(item)}><td><div className="item-name"><img className={`rarity-border ${item.rarity}`} src={asset(item.iconPath)} alt=""/><span><b>{pick(item.name,lang)}</b>{item.set&&<small>{pick(item.set,lang)}</small>}</span></div></td><td>{subtypeName(item.subtype,lang)}</td><td><div className="item-badges"><span className={`rarity ${item.rarity}`}>{rarityName(item.rarity,lang)}</span>{item.set&&<span className="set-tag">{copy(lang,'套装','Set','套裝')}</span>}</div></td><td>{item.level}</td><td>{item.requiredLevel||'—'}</td><td>{item.category==='socketable'?'—':item.sockets}</td><td><ChevronRight size={14}/></td></tr>)}</tbody></table></div>}<Pagination page={currentPage} pages={pages} setPage={setCurrentPage} lang={lang}/></div>{selected&&<EquipmentDrawer item={selected} variants={items.filter(item=>item.name.en===selected.name.en&&item.subtype===selected.subtype)} lang={lang} onClose={()=>setSelected(null)}/>}</>
}
...
function EquipmentDrawer({item,variants,lang,onClose}:{item:DbEquipment;variants:DbEquipment[];lang:Lang;onClose:()=>void}){
  ...
  const showRawEffects=!current.effects.length&&current.rawEffects.length>0
  return <div className="drawer-backdrop" onMouseDown={event=>{if(event.target===event.currentTarget)onClose()}}><aside className="detail-drawer"><button className="drawer-close" onClick={onClose} aria-label={tr(lang,'close')}><X/></button><div className="drawer-title"><img className={`rarity-border ${current.rarity}`} src={asset(current.iconPath)} alt=""/><div><div className="item-badges"><span className={`rarity ${current.rarity}`}>{rarityName(current.rarity,lang)}</span>{current.set&&<span className="set-tag">{copy(lang,'套装','Set','套裝')}</span>}</div><h2>{pick(current.name,lang)}</h2>{originalName(current.name,lang)&&<small className="original-name">{current.name.en}</small>}<p className="subtype">{subtypeName(current.subtype,lang)}</p></div></div>
    ...
    {current.description&&<blockquote>{pick(current.description,lang)}</blockquote>}<dl className="detail-grid"><div><dt>{tr(lang,'level')}</dt><dd>{current.level}</dd></div><div><dt>{tr(lang,'required')}</dt><dd>{current.requiredLevel||'—'}</dd></div>{current.category!=='socketable'&&<div><dt>{tr(lang,'sockets')}</dt><dd>{current.maxSockets?`${current.sockets} / ${current.maxSockets}`:current.sockets}</dd></div>}<div><dt>{copy(lang,'速度值','Speed value','速度值')}</dt><dd>{current.speed??'—'}</dd></div></dl>
    ...
    {current.effects.length>0&&<DetailSection title={copy(lang,'物品效果','Item effects','物品效果')}><ul className="effect-list">{current.effects.map((effect,index)=><li key={`${effect.text.en}-${index}`}>{pick(effect.text,lang)}</li>)}</ul></DetailSection>}
    ...
    {current.set&&<DetailSection title={copy(lang,'套装','Set','套裝')}><p>{pick(current.set,lang)}</p>{current.setBonuses.length>0?<ul className="effect-list set-bonuses">{current.setBonuses.map((bonus,index)=><li key={`${bonus.pieces}-${bonus.text.en}-${index}`}><b>{bonus.pieces} {copy(lang,'件','pieces','件')}</b>{pick(bonus.text,lang)}</li>)}</ul>:current.rawSetBonuses.length>0&&<ul className="raw-effect-list">{current.rawSetBonuses.map((bonus,index)=><RawEffectLine key={`${bonus.pieces}-${bonus.type}-${index}`} effect={bonus} lang={lang} pieces={bonus.pieces}/>)}</ul>}</DetailSection>}
...
function SearchOverlay({lang,query,setQuery,onClose,go,onClass,onSkill,data}:{lang:Lang;query:string;setQuery:(query:string)=>void;onClose:()=>void;go:(page:Page)=>void;onClass:(id:string)=>void;onSkill:(focus:SkillFocus)=>void;data:SiteData}){
  ...
  const results=useMemo(()=>{const needle=query.trim().toLowerCase();if(!needle)return[];const out:SearchResult[]=[]
    ...
    data.equipment.forEach(item=>{if(`${allText(item.name)} ${item.subtype} ${item.set?allText(item.set):''} ${item.effects.map(effect=>allText(effect.text)).join(' ')}`.toLowerCase().includes(needle))out.push({type:'item',name:pick(item.name,lang),sub:`${subtypeName(item.subtype,lang)} · Lv ${item.level}`,page:'items',image:item.iconPath})})
