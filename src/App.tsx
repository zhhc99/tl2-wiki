...
import {
  ArrowRight, BookOpen, ChevronDown, ChevronLeft, ChevronRight, Compass, Globe2,
...
interface DbEquipment {
  ...
  effects: DbEffect[]; rawEffects: RawEffect[]; setBonuses: DbSetBonus[]; rawSetBonuses: DbRawSetBonus[]
...
function SelectControl({label,value,onChange,children,className='',icon}:{label:string;value:string|number;onChange:ChangeEventHandler<HTMLSelectElement>;children:ReactNode;className?:string;icon?:ReactNode}){
  return <label className={`select-control${icon?' has-icon':''}${className?` ${className}`:''}`}>
    <span className="sr-only">{label}</span>{icon&&<span className="select-icon" aria-hidden="true">{icon}</span>}
    <select value={value} onChange={onChange} aria-label={label}>{children}</select><ChevronDown className="select-caret" size={14} aria-hidden="true"/>
  </label>
}
...
function Header({lang,setLang,page,go,mobileOpen,setMobileOpen,onSearch}:{lang:Lang;setLang:(lang:Lang)=>void;page:Page;go:(page:Page)=>void;mobileOpen:boolean;setMobileOpen:(open:boolean)=>void;onSearch:()=>void}){
  ...
  return <header className="site-header"><div className="nav-wrap">
    ...
    <div className="header-tools">
      ...
      <SelectControl className="locale-select" label={tr(lang,'chooseLanguage')} value={lang} onChange={event=>setLang(event.target.value as Lang)} icon={<Globe2 size={15}/>}>{localeOptions.map(option=><option key={option.code} value={option.code}>{option.label}</option>)}</SelectControl>
...
function Home({lang,go,onSearch,onClass,data}:{lang:Lang;go:(page:Page)=>void;onSearch:()=>void;onClass:(id:string)=>void;data:SiteData}){
  ...
  const links=[
    {page:'classes' as Page,icon:<Swords/>,title:tr(lang,'navClasses'),text:copy(lang,'4 个职业、12 棵技能树和 1,800 条等级数据','4 classes, 12 skill trees and 1,800 rank records','4 個職業、12 棵技能樹和 1,800 筆等級資料'),count:counts?.classSkills||120},
    {page:'items' as Page,icon:<Hammer/>,title:tr(lang,'navItems'),text:copy(lang,'按名称、类型、稀有度和等级筛选','Filter by name, type, rarity and level','按名稱、類型、稀有度和等級篩選'),count:counts?.equipment||5483},
  ...
  return <>
    <section className="home-hero"><div className="content home-hero-inner"><div><p className="kicker">TORCHLIGHT II</p><h1>TL2 Wiki</h1><p className="home-lead">{copy(lang,'查职业技能、装备属性、技能书和相位兽挑战。','Look up class skills, equipment, spell books and Phase Beast challenges.','查職業技能、裝備屬性、技能書和相位獸挑戰。')}</p><button className="home-search" onClick={onSearch}><Search size={20}/><span>{tr(lang,'search')}</span><kbd>Ctrl K</kbd></button></div>
      <div className="home-summary"><span>{copy(lang,'内容总览','At a glance','內容總覽')}</span><b>{(counts?.equipment||5483).toLocaleString()} {copy(lang,'件装备','items','件裝備')}</b><p>{counts?.classSkills||120} {copy(lang,'个职业技能','class skills','個職業技能')} · {(counts?.spellBooks||194).toLocaleString()} {copy(lang,'种技能书','spell books','種技能書')} · {counts?.phaseChallenges||15} {copy(lang,'项相位兽挑战','Phase Beast challenges','項相位獸挑戰')}</p></div>
    </div></section>
...
function ClassesPage({lang,classId,setClassId,classSkills,focus}:{lang:Lang;classId:string;setClassId:(id:string)=>void;classSkills:DbClassGroup[];focus:SkillFocus|null}){
  ...
  return <>
    <PageHeader section={tr(lang,'navClasses')} title={tr(lang,'classesTitle')}>{copy(lang,'选择职业和技能树，查看游戏说明、每级数值与阶段奖励。','Choose a class and skill tree to inspect its in-game description, rank values and tier bonuses.','選擇職業和技能樹，查看遊戲說明、每級數值與階段獎勵。')}</PageHeader>
...
function MechanicsPage({lang}:{lang:Lang}){
  ...
  return <><PageHeader section={tr(lang,'navMechanics')} title={tr(lang,'mechTitle')}>{copy(lang,'查看四项属性的准确收益，以及不同伤害能否触发暴击、吸取和武器效果。','See the exact returns from the four attributes and which damage sources can trigger critical hits, stealing and weapon effects.','查看四項屬性的準確收益，以及不同傷害能否觸發爆擊、吸取和武器效果。')}</PageHeader>
    <div className="content page-body"><div className="stat-grid">{statInfo.map(stat=><article className={`stat-summary ${stat.key}`} key={stat.key}><div><StatPill stat={stat.key}/><h3>{pick(stat.name,lang)}</h3></div><ul>{stat.effects.map((effect,index)=><li key={index}><RichText value={pick(effect,lang)}/></li>)}</ul></article>)}</div>
      <section className="socket-section"><SectionTitle eyebrow={copy(lang,'装备','Equipment','裝備')} title={copy(lang,'孔数规则','Socket rules','孔數規則')}/><div className="socket-rules"><article><b>2</b><div><h3>{copy(lang,'打孔师上限','Socket enchanter cap','打孔師上限')}</h3><p>{copy(lang,'打孔师 Jurick 可以为装备增加孔，普通装备最多达到 2 孔。','Jurick can add sockets to equipment, up to 2 sockets on ordinary items.','打孔師 Jurick 可以為裝備增加孔，普通裝備最多達到 2 孔。')}</p></div></article><article><b>5</b><div><h3>The Asphyx</h3><p>{copy(lang,'该套装的头盔和手套各有 5 孔。','Its helmet and gloves each have 5 sockets.','該套裝的頭盔和手套各有 5 孔。')}</p></div></article><article><b>3 / 4</b><div><h3>{copy(lang,'武器掉落变种','Weapon drop variants','武器掉落變體')}</h3><p>{copy(lang,'附魔绿色武器掉落时最多 3 孔；稀有蓝色武器最多 4 孔，盾牌不在此列。','Enchanted green weapons can drop with up to 3 sockets; rare blue weapons can have up to 4, excluding shields.','附魔綠色武器掉落時最多 3 孔；稀有藍色武器最多 4 孔，盾牌不在此列。')}</p></div></article><article><b>4 / 5</b><div><h3>Netherrealm</h3><p>{copy(lang,'该系列单手武器为 4 孔，双手武器为 5 孔；Netherrealm Shield 为 4 孔。','One-hand weapons have 4 sockets, two-hand weapons have 5, and Netherrealm Shield has 4.','該系列單手武器為 4 孔，雙手武器為 5 孔；Netherrealm Shield 為 4 孔。')}</p></div></article></div></section>
...
function ItemsPage({lang,items}:{lang:Lang;items:DbEquipment[]}){
  ...
  const filtered=useMemo(()=>items.filter(item=>(category==='all'||item.category===category)&&(rarity==='all'||item.rarity===rarity)&&(level==='all'||(level==='100'?item.level>=100:item.level>=Number(level)&&item.level<Number(level)+20))&&(`${allText(item.name)} ${item.subtype} ${item.set?allText(item.set):''} ${item.rawEffects.map(effect=>effect.text?allText(effect.text):'').join(' ')} ${item.effects.map(effect=>allText(effect.text)).join(' ')}`).toLowerCase().includes(query.toLowerCase())),[items,category,rarity,level,query])
  ...
  return <><PageHeader section={tr(lang,'navItems')} title={tr(lang,'itemsTitle')}>{copy(lang,'按名称、类型、稀有度、等级或物品效果查找装备。','Find equipment by name, type, rarity, level or item effect.','按名稱、類型、稀有度、等級或物品效果查找裝備。')}</PageHeader><div className="content page-body"><div className="data-toolbar"><label className="data-search"><Search size={16}/><input value={query} onChange={event=>setQuery(event.target.value)} placeholder={copy(lang,'搜索名称、类型、套装或效果…','Search name, type, set or effect…','搜尋名稱、類型、套裝或效果…')}/></label><SelectControl className="filter-select" label={copy(lang,'装备类型','Equipment type','裝備類型')} value={category} onChange={event=>setCategory(event.target.value as typeof category)}>{['all','weapon','armor','trinket','pet','socketable'].map(value=><option value={value} key={value}>{tr(lang,categoryKey(value))}</option>)}</SelectControl><SelectControl className="filter-select" label={copy(lang,'稀有度','Rarity','稀有度')} value={rarity} onChange={event=>setRarity(event.target.value as typeof rarity)}><option value="all">{tr(lang,'allRarity')}</option>{(['rare','unique','legendary'] as Rarity[]).map(value=><option value={value} key={value}>{rarityName(value,lang)}</option>)}</SelectControl><SelectControl className="filter-select" label={copy(lang,'装备等级','Item level','裝備等級')} value={level} onChange={event=>setLevel(event.target.value)}><option value="all">{copy(lang,'全部等级','All levels','全部等級')}</option>{[0,20,40,60,80,100].map(value=><option value={value} key={value}>Lv {value}{value<100?`–${value+19}`:'+'}</option>)}</SelectControl></div>
    <div className="result-meta"><span>{filtered.length.toLocaleString()} {tr(lang,'itemsFound')}</span><span>{copy(lang,`第 ${currentPage} / ${pages} 页`,`Page ${currentPage} of ${pages}`,`第 ${currentPage} / ${pages} 頁`)}</span></div>{!items.length?<Loading lang={lang}/>:<div className="table-scroll"><table className="data-table"><thead><tr><th>{copy(lang,'名称','Name','名稱')}</th><th>{copy(lang,'类型','Type','類型')}</th><th>{copy(lang,'稀有度','Rarity','稀有度')}</th><th>{tr(lang,'level')}</th><th>{tr(lang,'required')}</th><th/></tr></thead><tbody>{rows.map(item=><tr key={item.id} onClick={()=>setSelected(item)}><td><div className="item-name"><img className={`rarity-border ${item.rarity}`} src={asset(item.iconPath)} alt=""/><span><b>{pick(item.name,lang)}</b>{item.set&&<small>{pick(item.set,lang)}</small>}</span></div></td><td>{subtypeName(item.subtype,lang)}</td><td><div className="item-badges"><span className={`rarity ${item.rarity}`}>{rarityName(item.rarity,lang)}</span>{item.set&&<span className="set-tag">{copy(lang,'套装','Set','套裝')}</span>}</div></td><td>{item.level}</td><td>{item.requiredLevel||'—'}</td><td><ChevronRight size={14}/></td></tr>)}</tbody></table></div>}<Pagination page={currentPage} pages={pages} setPage={setCurrentPage} lang={lang}/></div>{selected&&<EquipmentDrawer item={selected} variants={items.filter(item=>item.name.en===selected.name.en&&item.subtype===selected.subtype)} lang={lang} onClose={()=>setSelected(null)}/>}</>
}
...
function EquipmentDrawer({item,variants,lang,onClose}:{item:DbEquipment;variants:DbEquipment[];lang:Lang;onClose:()=>void}){
  ...
  const showRawEffects=current.rawEffects.length>0
  return <div className="drawer-backdrop" onMouseDown={event=>{if(event.target===event.currentTarget)onClose()}}><aside className="detail-drawer"><button className="drawer-close" onClick={onClose} aria-label={tr(lang,'close')}><X/></button><div className="drawer-title"><img className={`rarity-border ${current.rarity}`} src={asset(current.iconPath)} alt=""/><div><div className="item-badges"><span className={`rarity ${current.rarity}`}>{rarityName(current.rarity,lang)}</span>{current.set&&<span className="set-tag">{copy(lang,'套装','Set','套裝')}</span>}</div><h2>{pick(current.name,lang)}</h2>{originalName(current.name,lang)&&<small className="original-name">{current.name.en}</small>}<p className="subtype">{subtypeName(current.subtype,lang)}</p></div></div>
    {variants.length>1&&<div className="variant-field"><span>{copy(lang,'选择等级','Choose level','選擇等級')}</span><SelectControl className="variant-select" label={copy(lang,'选择等级','Choose level','選擇等級')} value={currentId} onChange={event=>setCurrentId(event.target.value)}>{[...variants].sort((a,b)=>a.level-b.level).map(variant=><option key={variant.id} value={variant.id}>Lv {variant.level} · {rarityName(variant.rarity,lang)}</option>)}</SelectControl></div>}
    {current.description&&<blockquote>{pick(current.description,lang)}</blockquote>}<dl className="detail-grid"><div><dt>{tr(lang,'level')}</dt><dd>{current.level}</dd></div><div><dt>{tr(lang,'required')}</dt><dd>{current.requiredLevel||'—'}</dd></div>{current.category==='weapon'&&current.speed!=null&&<div><dt>{copy(lang,'基础攻击间隔','Base attack interval','基礎攻擊間隔')}</dt><dd>{current.speed} {tr(lang,'seconds')}</dd></div>}</dl>
    {(current.requirements.length>0||current.classRequirement)&&<DetailSection title={copy(lang,'装备需求','Requirements','裝備需求')}>{current.requirements.length>0&&<div className="requirement-row">{current.requirements.map(requirement=><span key={requirement.stat}><StatPill stat={requirement.stat}/><b>{requirement.value}</b></span>)}</div>}{current.classRequirement&&<p>{copy(lang,'职业','Class','職業')}: {current.classRequirement}</p>}</DetailSection>}
    {!showRawEffects&&current.effects.length>0&&<DetailSection title={copy(lang,'物品效果','Item effects','物品效果')}><ul className="effect-list">{current.effects.map((effect,index)=><li key={`${effect.text.en}-${index}`}>{pick(effect.text,lang)}</li>)}</ul></DetailSection>}
    {showRawEffects&&<DetailSection title={copy(lang,'物品效果','Item effects','物品效果')}><ul className="raw-effect-list">{current.rawEffects.map((effect,index)=><RawEffectLine key={`${effect.type}-${index}`} effect={effect} lang={lang}/>)}</ul></DetailSection>}
...
function SearchOverlay({lang,query,setQuery,onClose,go,onClass,onSkill,data}:{lang:Lang;query:string;setQuery:(query:string)=>void;onClose:()=>void;go:(page:Page)=>void;onClass:(id:string)=>void;onSkill:(focus:SkillFocus)=>void;data:SiteData}){
  ...
  const results=useMemo(()=>{const needle=query.trim().toLowerCase();if(!needle)return[];const out:SearchResult[]=[]
    ...
    data.equipment.forEach(item=>{if(`${allText(item.name)} ${item.subtype} ${item.set?allText(item.set):''} ${item.rawEffects.map(effect=>effect.text?allText(effect.text):'').join(' ')} ${item.effects.map(effect=>allText(effect.text)).join(' ')}`.toLowerCase().includes(needle))out.push({type:'item',name:pick(item.name,lang),sub:`${subtypeName(item.subtype,lang)} · Lv ${item.level}`,page:'items',image:item.iconPath})})
