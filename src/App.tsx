...
function ItemsPage({lang,items}:{lang:Lang;items:DbEquipment[]}){
  ...
  const filtered=useMemo(()=>items.filter(item=>(category==='all'||item.category===category)&&(rarity==='all'||item.rarity===rarity)&&(level==='all'||(level==='100'?item.level>=100:item.level>=Number(level)&&item.level<Number(level)+20))&&(`${allText(item.name)} ${item.subtype} ${item.set?allText(item.set):''} ${item.effects.map(effect=>effect.text?allText(effect.text):'').join(' ')}`).toLowerCase().includes(query.toLowerCase())),[items,category,rarity,level,query])
...
function SearchOverlay({lang,query,setQuery,onClose,go,onClass,onSkill,data}:{lang:Lang;query:string;setQuery:(query:string)=>void;onClose:()=>void;go:(page:Page)=>void;onClass:(id:string)=>void;onSkill:(focus:SkillFocus)=>void;data:SiteData}){
  ...
  const results=useMemo(()=>{const needle=query.trim().toLowerCase();if(!needle)return[];const out:SearchResult[]=[]
    ...
    data.equipment.forEach(item=>{if(`${allText(item.name)} ${item.subtype} ${item.set?allText(item.set):''} ${item.effects.map(effect=>effect.text?allText(effect.text):'').join(' ')}`.toLowerCase().includes(needle))out.push({type:'item',name:pick(item.name,lang),sub:`${subtypeName(item.subtype,lang)} · Lv ${item.level}`,page:'items',image:item.iconPath})})
