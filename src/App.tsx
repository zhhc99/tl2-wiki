...
function Header({lang,setLang,page,go,mobileOpen,setMobileOpen,onSearch}:{lang:Lang;setLang:(lang:Lang)=>void;page:Page;go:(page:Page)=>void;mobileOpen:boolean;setMobileOpen:(open:boolean)=>void;onSearch:()=>void}){
  ...
  return <header className="site-header"><div className="nav-wrap">
    ...
    <div className="header-tools">
      ...
      <label className="locale-select"><Globe2 size={15}/><span className="sr-only">{tr(lang,'chooseLanguage')}</span><select value={lang} onChange={event=>setLang(event.target.value as Lang)} aria-label={tr(lang,'chooseLanguage')}>{localeOptions.map(option=><option key={option.code} value={option.code}>{option.label}</option>)}</select><ChevronDown size={13}/></label>
...
function rarityName(rarity:Rarity,lang:Lang){const names:Record<Rarity,LocalText>={normal:text('Normal','普通','普通'),rare:text('Rare','稀有','稀有'),set:text('Set','套装','套裝'),unique:text('Unique','独特','獨特'),legendary:text('Legendary','传奇','傳奇')};return pick(names[rarity],lang)}
...
function ItemsPage({lang,items}:{lang:Lang;items:DbEquipment[]}){
  ...
  return <><PageHeader section={tr(lang,'navItems')} title={tr(lang,'itemsTitle')}>{copy(lang,'按名称、类型、稀有度、等级或物品效果查找装备。','Find equipment by name, type, rarity, level or item effect.','按名稱、類型、稀有度、等級或物品效果查找裝備。')}</PageHeader><div className="content page-body"><div className="data-toolbar"><label className="data-search"><Search size={16}/><input value={query} onChange={event=>setQuery(event.target.value)} placeholder={copy(lang,'搜索名称、类型、套装或效果…','Search name, type, set or effect…','搜尋名稱、類型、套裝或效果…')}/></label><select value={category} onChange={event=>setCategory(event.target.value as typeof category)}>{['all','weapon','armor','trinket','pet','socketable'].map(value=><option value={value} key={value}>{tr(lang,categoryKey(value))}</option>)}</select><select value={rarity} onChange={event=>setRarity(event.target.value as typeof rarity)}><option value="all">{tr(lang,'allRarity')}</option>{(['normal','rare','set','unique','legendary'] as Rarity[]).map(value=><option value={value} key={value}>{rarityName(value,lang)}</option>)}</select><select value={level} onChange={event=>setLevel(event.target.value)}><option value="all">{copy(lang,'全部等级','All levels','全部等級')}</option>{[0,20,40,60,80,100].map(value=><option value={value} key={value}>Lv {value}{value<100?`–${value+19}`:'+'}</option>)}</select></div>
    <div className="result-meta"><span>{filtered.length.toLocaleString()} {tr(lang,'itemsFound')}</span><span>{copy(lang,`第 ${currentPage} / ${pages} 页`,`Page ${currentPage} of ${pages}`,`第 ${currentPage} / ${pages} 頁`)}</span></div>{!items.length?<Loading lang={lang}/>:<div className="table-scroll"><table className="data-table"><thead><tr><th>{copy(lang,'名称','Name','名稱')}</th><th>{copy(lang,'类型','Type','類型')}</th><th>{copy(lang,'稀有度','Rarity','稀有度')}</th><th>{tr(lang,'level')}</th><th>{tr(lang,'required')}</th><th>{tr(lang,'sockets')}</th><th/></tr></thead><tbody>{rows.map(item=><tr key={item.id} onClick={()=>setSelected(item)}><td><div className="item-name"><img src={asset(item.iconPath)} alt=""/><span><b>{pick(item.name,lang)}</b>{item.set&&<small>{pick(item.set,lang)}</small>}</span></div></td><td>{subtypeName(item.subtype,lang)}</td><td><span className={`rarity ${item.rarity}`}>{rarityName(item.rarity,lang)}</span></td><td>{item.level}</td><td>{item.requiredLevel||'—'}</td><td>{item.category==='socketable'?'—':item.sockets}</td><td><ChevronRight size={14}/></td></tr>)}</tbody></table></div>}<Pagination page={currentPage} pages={pages} setPage={setCurrentPage} lang={lang}/></div>{selected&&<EquipmentDrawer item={selected} variants={items.filter(item=>item.name.en===selected.name.en&&item.subtype===selected.subtype)} lang={lang} onClose={()=>setSelected(null)}/>}</>
}
...
function EquipmentDrawer({item,variants,lang,onClose}:{item:DbEquipment;variants:DbEquipment[];lang:Lang;onClose:()=>void}){
  ...
  return <div className="drawer-backdrop" onMouseDown={event=>{if(event.target===event.currentTarget)onClose()}}><aside className="detail-drawer"><button className="drawer-close" onClick={onClose} aria-label={tr(lang,'close')}><X/></button><div className="drawer-title"><img src={asset(current.iconPath)} alt=""/><div><span className={`rarity ${current.rarity}`}>{rarityName(current.rarity,lang)}</span><h2>{pick(current.name,lang)}</h2>{originalName(current.name,lang)&&<small className="original-name">{current.name.en}</small>}<p className="subtype">{subtypeName(current.subtype,lang)}</p></div></div>
    {variants.length>1&&<label className="variant-select"><span>{copy(lang,'选择等级','Choose level','選擇等級')}</span><select value={currentId} onChange={event=>setCurrentId(event.target.value)}>{[...variants].sort((a,b)=>a.level-b.level).map(variant=><option key={variant.id} value={variant.id}>Lv {variant.level} · {rarityName(variant.rarity,lang)}</option>)}</select></label>}
...
function SpellsPage({lang,spells}:{lang:Lang;spells:DbSpellBook[]}){
  ...
  return <><PageHeader section={tr(lang,'navSpells')} title={tr(lang,'spellsTitle')}>{copy(lang,'选择技能书，查看可用等级、需求与游戏说明。','Choose a spell book to see its tiers, requirements and in-game description.','選擇技能書，查看可用等級、需求與遊戲說明。')}</PageHeader><div className="content page-body"><div className="data-toolbar"><label className="data-search"><Search size={16}/><input value={query} onChange={event=>setQuery(event.target.value)} placeholder={copy(lang,'搜索技能书…','Search spell books…','搜尋技能書…')}/></label><select value={school} onChange={event=>setSchool(event.target.value as typeof school)}><option value="all">{tr(lang,'all')}</option>{(['offense','defense','summon','utility'] as DbSpellBook['school'][]).map(value=><option value={value} key={value}>{tr(lang,value)}</option>)}</select></div><div className="result-meta"><span>{copy(lang,`显示 ${families.length} 种技能书`,`Showing ${families.length} spell books`,`顯示 ${families.length} 種技能書`)}</span></div>{!spells.length?<Loading lang={lang}/>:<div className="spell-families">{families.map(([family,tiers])=><article key={family}><img className="spell-icon" src={asset(tiers[0].iconPath)} alt=""/><div><span className={`school ${tiers[0].school}`}>{tr(lang,tiers[0].school)}</span><h2>{pick(tiers[0].family,lang)}</h2>{originalName(tiers[0].family,lang)&&<small className="original-name">{tiers[0].family.en}</small>}<p>{pick(tiers[0].description,lang)}</p></div><div className="tier-list">{tiers.map(tier=><button key={tier.id} onClick={()=>setSelected(tier)}><b>{tier.tier}</b><span>Lv {tier.requiredLevel||tier.level}</span></button>)}</div></article>)}</div>}</div>{selected&&<SpellDrawer spell={selected} lang={lang} onClose={()=>setSelected(null)}/>}</>
}
