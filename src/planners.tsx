...
export function BuildsPage({lang,items}:{lang:Lang;items:PlannerEquipment[]}){
  ...
  useEffect(()=>{
    ...
    setSocketLoadout(current=>{
      ...
      for(const slot of slots){
        ...
        const values=(current[slot]||[]).slice(0,item?.sockets||0).map(id=>id&&byId.get(id)?.category==='socketable'?id:null)
  ...
  const activeSocketRows=equipped.flatMap(({slot,item})=>(socketLoadout[slot]||[]).slice(0,item.sockets).flatMap((gemId,index)=>{
  ...
  const previewSockets=preview&&!candidate?Array.from({length:preview.item.sockets},(_,index)=>{
  ...
  return <><section className="page-header"><div className="content"><span>{copy(lang,'角色','Character','角色')}</span><h1>{copy(lang,'配装','Build planner','配裝')}</h1><p>{copy(lang,'选择职业、分配属性点并穿戴装备，集中查看属性、基础数值和装备需求。','Choose a class, allocate attribute points and equip a full loadout to inspect stats, base values and requirements.','選擇職業、分配屬性點並穿上裝備，集中查看屬性、基礎數值與裝備需求。')}</p></div></section>
    <div className="content page-body build-page">
      ...
      {activeSocketRows.length>0&&<section className="socketed-gems"><header><div><span>{copy(lang,'镶嵌','Sockets','鑲嵌')}</span><h2>{copy(lang,'已镶嵌宝石','Socketed gems','已鑲嵌寶石')}</h2></div><strong>{activeSocketRows.length}</strong></header><div className="socketed-gem-list">{activeSocketRows.map(row=><article key={`${row.slot}-${row.index}`}><img src={asset(row.gem.iconPath)} alt=""/><div><span>{slotName(row.slot,lang)} · {copy(lang,`第 ${row.index+1} 孔`,`Socket ${row.index+1}`,`第 ${row.index+1} 孔`)}</span><b>{pick(row.gem.name,lang)}</b><ul>{row.effects.map((effect,index)=><li key={`${effect.type}-${index}`}>{effect.text?pick(effect.text,lang):effect.type}</li>)}</ul></div></article>)}</div></section>}
    </div>
    {preview&&<div className="gear-preview-backdrop" onMouseDown={event=>{if(event.target===event.currentTarget)closePreview()}}>
      <section className="gear-preview" role="dialog" aria-modal="true" aria-label={copy(lang,'装备速览','Equipment quick view','裝備快速預覽')}>
        <header><div><span>{slotName(preview.slot,lang)} · {candidate?copy(lang,'装备前预览','Preview before equipping','裝備前預覽'):copy(lang,'速览','Quick view','快速預覽')}</span><h2>{pick(preview.item.name,lang)}</h2></div><button onClick={closePreview} aria-label={copy(lang,'关闭','Close','關閉')}><X/></button></header>
