...
import { CircleAlert, CircleDollarSign, Eye, RefreshCw, RotateCcw, Search, Shield, Swords, X } from 'lucide-react'
...
export function BuildsPage({lang,items}:{lang:Lang;items:PlannerEquipment[]}){
  ...
  const requirements=equipped.map(({slot,item})=>{
    ...
    const ownStats=item.effects.reduce((total,effect)=>{const stat=statEffectTypes[effect.type];if(stat)total[stat]+=fixedEffectValue(effect);return total},{str:0,dex:0,foc:0,vit:0} as Record<Stat,number>)
  ...
  return <><section className="page-header"><div className="content"><span>{copy(lang,'角色','Character','角色')}</span><h1>{copy(lang,'配装','Build planner','配裝')}</h1><p>{copy(lang,'选择职业、分配属性点并穿戴装备，集中查看属性、基础数值和装备需求。','Choose a class, allocate attribute points and equip a full loadout to inspect stats, base values and requirements.','選擇職業、分配屬性點並穿上裝備，集中查看屬性、基礎數值與裝備需求。')}</p></div></section>
    ...
    {preview&&<div className="gear-preview-backdrop" onMouseDown={event=>{if(event.target===event.currentTarget)closePreview()}}><section className="gear-preview" role="dialog" aria-modal="true" aria-label={copy(lang,'装备速览','Equipment quick view','裝備快速預覽')}><header><div><span>{slotName(preview.slot,lang)} · {candidate?copy(lang,'装备前预览','Preview before equipping','裝備前預覽'):copy(lang,'速览','Quick view','快速預覽')}</span><h2>{pick(preview.item.name,lang)}</h2></div><button onClick={closePreview} aria-label={copy(lang,'关闭','Close','關閉')}><X/></button></header><div className="gear-preview-title"><img className={`rarity-border ${preview.item.rarity}`} src={asset(preview.item.iconPath)} alt=""/><div><b>Lv {preview.item.level}</b>{preview.item.set&&<span>{pick(preview.item.set,lang)}</span>}<div className="gear-preview-requirements"><small className={level>=preview.item.requiredLevel?'ok':'blocked'}>{copy(lang,`需求等级 ${preview.item.requiredLevel||'—'}`,`Requires level ${preview.item.requiredLevel||'—'}`,`需求等級 ${preview.item.requiredLevel||'—'}`)}</small>{previewRequirements.map(requirement=><small className={requirement.ok?'ok':'blocked'} key={requirement.stat}>{pick(statNames[requirement.stat],lang)} {requirement.value}</small>)}</div></div></div><div className="gear-preview-values">{Boolean(previewDamage[0]||previewDamage[1])&&<div><span>{copy(lang,'基础伤害','Base damage','基礎傷害')}</span><b>{Math.round(previewDamage[0])}–{Math.round(previewDamage[1])}</b></div>}{Boolean(previewArmor[0]||previewArmor[1])&&<div><span>{copy(lang,'基础护甲','Base armor','基礎護甲')}</span><b>{Math.round(previewArmor[0])}–{Math.round(previewArmor[1])}</b></div>}</div>{preview.item.effects.length>0&&<section><h3>{copy(lang,'装备效果','Equipment effects','裝備效果')}</h3><ul>{preview.item.effects.map((effect,index)=><li key={`${effect.type}-${index}`}>{effect.text?pick(effect.text,lang):effect.type}</li>)}</ul></section>}<footer>{candidate?<button className="preview-replace" onClick={equipCandidate}><Shield size={16}/>{copy(lang,'装备到栏位','Equip item','裝備至欄位')}</button>:<button className="preview-replace" onClick={()=>{setPicker(preview.slot);setQuery('');closePreview()}}><RefreshCw size={16}/>{copy(lang,'更换装备','Replace item','更換裝備')}</button>}<button onClick={closePreview}>{copy(lang,'关闭','Close','關閉')}</button></footer></section></div>}
