...
import { ChevronDown, CircleDollarSign, RotateCcw, Search, Shield, Swords, X } from 'lucide-react'
...
export interface PlannerEquipment {
  ...
  rawEffects:PlannerEffect[]; rawSetBonuses:(PlannerEffect&{pieces:number})[]
}
...
function Select({label,value,onChange,children}:{label:string;value:string|number;onChange:(value:string)=>void;children:React.ReactNode}){
  return <label className="select-control planner-select"><span className="sr-only">{label}</span><select aria-label={label} value={value} onChange={event=>onChange(event.target.value)}>{children}</select><ChevronDown className="select-caret" size={14}/></label>
}
...
export function BuildsPage({lang,items}:{lang:Lang;items:PlannerEquipment[]}){
  ...
  const equipmentEffects=[...equipped.flatMap(row=>row.item.rawEffects),...activeSetEffects]
  ...
  const requirements=equipped.map(({slot,item})=>{
    ...
    const ownStats=item.rawEffects.reduce((total,effect)=>{const stat=statEffectTypes[effect.type];if(stat&&effect.activation==='PASSIVE'&&effect.min!=null&&effect.min===effect.max)total[stat]+=effect.min;return total},{str:0,dex:0,foc:0,vit:0} as Record<Stat,number>)
  ...
  const reset=()=>{setClassId('berserker');setLevel(100);setAllocated({str:0,dex:0,foc:0,vit:0});setLoadout(emptyLoadout())}

  return <><section className="page-header"><div className="content"><span>{copy(lang,'角色','Character','角色')}</span><h1>{copy(lang,'配装','Build planner','配裝')}</h1><p>{copy(lang,'选择职业、分配属性点并穿戴装备，集中查看属性、基础数值和装备需求。','Choose a class, allocate attribute points and equip a full loadout to inspect stats, base values and requirements.','選擇職業、分配屬性點並穿戴裝備，集中查看屬性、基礎數值和裝備需求。')}</p></div></section>
    <div className="content page-body build-page">
      <div className="build-toolbar"><Select label={copy(lang,'职业','Class','職業')} value={classId} onChange={value=>setClassId(value)}>{classes.map(hero=><option value={hero.id} key={hero.id}>{pick(hero.name,lang)}</option>)}</Select><label className="level-input"><span>{copy(lang,'角色等级','Character level','角色等級')}</span><input type="number" min="1" max="100" value={level} onChange={event=>setLevel(Math.max(1,Math.min(100,Number(event.target.value)||1)))}/></label><button className="reset-build" onClick={reset}><RotateCcw size={16}/>{copy(lang,'重置','Reset','重設')}</button></div>
      <div className="build-layout"><section className="paper-doll"><header><div><span>{copy(lang,'装备栏','Equipment','裝備欄')}</span><h2>{pick(classes.find(hero=>hero.id===classId)?.name||classes[0].name,lang)}</h2></div><strong>{equipped.length} / 12</strong></header><div className="slot-grid">{slots.map(slot=>{const item=loadout[slot]?byId.get(loadout[slot] as string):null;const locked=slot==='off'&&Boolean(loadout.main&&twoHanded.has(byId.get(loadout.main)?.subtype||''));return <button key={slot} className={`gear-slot ${item?'filled':''}`} disabled={locked} onClick={()=>{setPicker(slot);setQuery('')}}><span className="slot-label">{slotName(slot,lang)}</span>{item?<><img className={`rarity-border ${item.rarity}`} src={asset(item.iconPath)} alt=""/><span className="slot-item"><b>{pick(item.name,lang)}</b><small>Lv {item.level}</small></span><X className="remove-item" size={15} onClick={event=>{event.stopPropagation();setLoadout(current=>({...current,[slot]:null}))}}/></>:<><span className="slot-empty">{locked?copy(lang,'双手武器占用','Occupied by two-hand weapon','雙手武器佔用'):copy(lang,'选择装备','Choose item','選擇裝備')}</span></>}</button>})}</div></section>
        <aside className="build-inspector"><section className="allocation-card"><header><div><span>{copy(lang,'属性加点','Attributes','屬性加點')}</span><b className={spent>available?'over':''}>{spent} / {available}</b></div><div className="point-track"><i style={{width:`${Math.min(100,available?spent/available*100:0)}%`}}/></div></header>{(Object.keys(statNames) as Stat[]).map(stat=><label className={`stat-allocation ${stat}`} key={stat}><span><b>{pick(statNames[stat],lang)}</b><small>{classBases[classId][stat]} + {gearStats[stat]} {copy(lang,'装备','gear','裝備')}</small></span><input aria-label={pick(statNames[stat],lang)} type="number" min="0" max="495" value={allocated[stat]} onChange={event=>setAllocated(current=>({...current,[stat]:Math.max(0,Math.min(495,Number(event.target.value)||0))}))}/><strong>{stats[stat]}</strong></label>)}{spent>available&&<p className="build-warning">{copy(lang,`超出当前等级可分配点数 ${spent-available} 点。`,`Allocation exceeds the level limit by ${spent-available}.`,`超出目前等級可分配點數 ${spent-available} 點。`)}</p>}</section>
          <section className="derived-card"><h2>{copy(lang,'属性检视','Stat overview','屬性檢視')}</h2><div className="derived-grid"><div><span>{copy(lang,'武器基础伤害','Base weapon damage','武器基礎傷害')}</span><b>{damage[0]||damage[1]?`${Math.round(damage[0])}–${Math.round(damage[1])}`:'—'}</b></div><div><span>{copy(lang,'基础护甲','Base armor','基礎護甲')}</span><b>{armor[0]||armor[1]?`${Math.round(armor[0])}–${Math.round(armor[1])}`:'—'}</b></div><div><span>{copy(lang,'武器伤害加成','Weapon damage','武器傷害加成')}</span><b>+{(stats.str*.5).toFixed(1)}%</b></div><div><span>{copy(lang,'暴击伤害加成','Critical damage','爆擊傷害加成')}</span><b>+{(stats.str*.4).toFixed(1)}%</b></div><div><span>{copy(lang,'暴击 / 闪避','Critical / dodge','爆擊 / 閃避')}</span><b>{chance(stats.dex).toFixed(1)}%</b></div><div><span>{copy(lang,'元素伤害加成','Elemental damage','元素傷害加成')}</span><b>+{(stats.foc*.5).toFixed(1)}%</b></div><div><span>{copy(lang,'额外法力','Added mana','額外法力')}</span><b>+{(stats.foc*.5).toFixed(1)}</b></div><div><span>{copy(lang,'处决几率','Execute chance','處決機率')}</span><b>{chance(stats.foc).toFixed(1)}%</b></div><div><span>{copy(lang,'额外生命','Added health','額外生命')}</span><b>+{Math.round(stats.vit*3.6)}</b></div><div><span>{copy(lang,'护甲加成','Armor bonus','護甲加成')}</span><b>+{(stats.vit*.25).toFixed(1)}%</b></div><div><span>{copy(lang,'格挡几率','Block chance','格擋機率')}</span><b>{shieldEquipped?`${chance(stats.vit).toFixed(1)}%`:'—'}</b></div></div></section>
          {(requirements.length>0||setCounts.size>0)&&<section className="build-status"><h2>{copy(lang,'当前配置','Current loadout','目前配置')}</h2>{requirements.map(({slot,item,ok})=><div key={slot}><span>{slotName(slot,lang)} · {pick(item.name,lang)}</span><b className={ok?'ok':'blocked'}>{ok?copy(lang,'可装备','Ready','可裝備'):copy(lang,'未满足需求','Requirements not met','未滿足需求')}</b></div>)}{[...setCounts.entries()].map(([name,count])=><div key={name}><span>{pick(equipped.find(row=>row.item.set?.en===name)?.item.set as LocalText,lang)}</span><b>{count} {copy(lang,'件','pieces','件')}</b></div>)}</section>}
        </aside></div>
      {effectSummary.length>0&&<section className="build-effects"><header><div><span>{copy(lang,'当前加成','Current bonuses','目前加成')}</span><h2>{copy(lang,'装备效果汇总','Equipment effects','裝備效果彙總')}</h2></div><strong>{effectSummary.length}</strong></header><ul>{effectSummary.map(([label,count])=><li key={label}><span>{label}</span>{count>1&&<b>× {count}</b>}</li>)}</ul></section>}
    </div>
    {picker&&<div className="picker-backdrop" onMouseDown={event=>{if(event.target===event.currentTarget)setPicker(null)}}><section className="item-picker"><header><div><span>{slotName(picker,lang)}</span><h2>{copy(lang,'选择装备','Choose equipment','選擇裝備')}</h2></div><button onClick={()=>setPicker(null)} aria-label={copy(lang,'关闭','Close','關閉')}><X/></button></header><label className="picker-search"><Search size={17}/><input autoFocus value={query} onChange={event=>setQuery(event.target.value)} placeholder={copy(lang,'搜索装备名称…','Search equipment…','搜尋裝備名稱…')}/></label><div className="picker-list"><button className="clear-slot" onClick={()=>{setLoadout(current=>({...current,[picker]:null}));setPicker(null)}}>{copy(lang,'清空这个栏位','Clear this slot','清空這個欄位')}</button>{pickerItems.map(item=><button key={item.id} onClick={()=>choose(item)}><img className={`rarity-border ${item.rarity}`} src={asset(item.iconPath)} alt=""/><span><b>{pick(item.name,lang)}</b><small>Lv {item.level}{item.set?` · ${pick(item.set,lang)}`:''}</small></span><em className={`rarity-dot ${item.rarity}`}/></button>)}{!pickerItems.length&&<p>{copy(lang,'没有匹配装备。','No matching equipment.','沒有符合的裝備。')}</p>}</div></section></div>}</>
}
...
export function GamblingPage({lang}:{lang:Lang}){
  ...
  return <><section className="page-header"><div className="content"><span>{copy(lang,'商店','Shop','商店')}</span><h1>{copy(lang,'赌博','Gambling','賭博')}</h1><p>{copy(lang,'赌博商人的标价由物品类型、物品等级和孔数决定。输入条件即可查看全部八个整数价格。','Gambler prices depend on item type, item level and sockets. Enter the item details to see all eight integer outcomes.','賭博商人的標價由物品類型、物品等級和孔數決定。輸入條件即可查看全部八個整數價格。')}</p></div></section><div className="content page-body gamble-page"><div className="gamble-layout"><section className="gamble-controls"><div className="gamble-mark"><CircleDollarSign/><span>{copy(lang,'价格计算器','Price calculator','價格計算器')}</span></div><label><span>{copy(lang,'物品类型','Item type','物品類型')}</span><Select label={copy(lang,'物品类型','Item type','物品類型')} value={type} onChange={setType}>{gambleTypes.map(row=><option key={row[0]} value={row[0]}>{copy(lang,row[3],row[2],row[4])}</option>)}</Select></label><label><span>{copy(lang,'物品等级','Item level','物品等級')}</span><input type="number" min="1" max="100" value={level} onChange={event=>setLevel(Math.max(1,Math.min(100,Number(event.target.value)||1)))}/></label><label><span>{copy(lang,'孔数','Sockets','孔數')}</span><input type="number" min="0" max="5" value={sockets} onChange={event=>setSockets(Math.max(0,Math.min(5,Number(event.target.value)||0)))}/></label><div className="formula"><span>{copy(lang,'计算式','Formula','計算式')}</span><code>{copy(lang,`(等级 + 4…7) × ${selected[1]} × (1 + 孔数 ÷ 10)`,`(level + 4…7) × ${selected[1]} × (1 + sockets ÷ 10)`,`(等級 + 4…7) × ${selected[1]} × (1 + 孔數 ÷ 10)`)}</code></div></section><section className="price-results"><header><div><span>{copy(lang,'可能价格','Possible prices','可能價格')}</span><h2>{copy(lang,selected[3],selected[2],selected[4])} · Lv {level}</h2></div><strong>× {selected[1]}</strong></header><div className="price-table"><div className="price-heading"><span>{copy(lang,'价格档','Band','價格檔')}</span><span>{copy(lang,'向下取整','Rounded down','向下取整')}</span><span>{copy(lang,'向上取整','Rounded up','向上取整')}</span></div>{rows.map(row=><div className="price-row" key={row.offset}><span>Lv + {row.offset}<small>{row.raw.toFixed(2)}</small></span><b>{row.low.toLocaleString()}</b><b>{row.high.toLocaleString()}</b></div>)}</div><p>{copy(lang,'共有四个底层价格；游戏取整时可能向下或向上，因此列出八个整数结果。两种取整相同时，会看到重复价格。','There are four underlying values. Each may be rounded down or up, producing eight integer outcomes; identical rounding results appear twice.','共有四個底層價格；遊戲取整時可能向下或向上，因此列出八個整數結果。兩種取整相同時，會看到重複價格。')}</p></section></div><section className="gamble-notes"><Swords/><div><h2>{copy(lang,'怎么算出来','How the price is formed','如何算出價格')}</h2><p>{copy(lang,'物品类型决定基础系数；每个孔会把价格提高 10%。商人再分别使用物品等级加 4、5、6、7，得到四档标价。','Item type sets the base factor and every socket adds 10%. The gambler then uses item level plus 4, 5, 6 and 7 for the four price bands.','物品類型決定基礎係數；每個孔會把價格提高 10%。商人再分別使用物品等級加 4、5、6、7，得到四檔標價。')}</p></div><Shield/></section></div></>
}
