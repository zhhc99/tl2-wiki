...
- `public/data/equipment.json`：3,840 件蓝色及以上稀有度装备、饰品、宠物装备与镶嵌物，包含 6,446 条应用继承覆盖和等级曲线后的最终有效三语效果。
...
`scripts/item-enrichment.json` 保留跨库规范化得到的最终伤害/护甲范围、属性需求、攻击速度和少量来源说明；当前页面中的 3,242 件装备可精确匹配这些补充数值。装备效果不再读取该旧快照，而是直接采用 schema v5 的 `item_display_effects`。`scripts/spell-books-source.json` 只保留 194 条技能书的分组、层级和类型分类；名称、说明、等级、需求与图标均按 GUID 读取主库。这两份规范化补充数据均已提交。
